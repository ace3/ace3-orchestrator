...
async function runAndWait(taskId, label) {
  const run = await api(`/api/tasks/${taskId}/run`, { method: "POST" });
  ...
  if (current.status !== "done") {
    let events = [];
    try { events = await api(`/api/runs/${run.id}/events`); } catch { /* best effort */ }
    const tail = events.slice(-8).map((e) => `    ${e.level || "?"}: ${e.message || ""}`).join("\n");
    throw new Error(
      `[${label}] run ${run.id} ended as "${current.status}" after ${runTimeoutSeconds}s.\n` +
      `last events:\n${tail || "    (none)"}`
    );
  }
...
async function main() {
  ...
  assert(parent && parent.status === "done", `[run#1] parent status=${parent && parent.status}, want "done"`);
  assert(
  ...
  projectTasks = await api(`/api/projects/${project.id}/tasks`);
